...
def _unavailable_measures(data: dict) -> set[str]:
    ...
    for key, m in MEASURES.items():
        for df in (data["county_df"], data["state_df"]):
            sample = df.head(1)
            try:
                m.values(sample)
...
def render_welcome() -> None:
    ...
    st.markdown("### Welcome")
    st.markdown(
        "Explore county-level Census data without needing a statistics background. "
    ...
    with st.expander("American Community Survey (ACS)", expanded=False):
        st.markdown(
            ...
            "of households. Unlike the once-a-decade Census, it uses a sample of "
    ...
    with st.expander("Which counties are statistically similar to mine?"):
        st.markdown(
            "Use **Statistical peers** below the county cards. It identifies counties whose "
...
def render_explorer(data: dict) -> None:
    ...
    if level == "county" and code_peek:
        ...
        if peeked_measure and peeked_measure not in unavailable:
