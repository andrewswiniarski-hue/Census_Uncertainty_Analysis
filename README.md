...
Raw data is never committed — scripts rebuild it identically on any machine. Run the scripts in [`/ingestion`](ingestion) (each one's docstring says what it needs and produces); outputs land in `data/raw/`. *(First pull script arrives with Phase 1, Step 4 — see [NEXT_ACTIONS.md](NEXT_ACTIONS.md) for live project status.)*
