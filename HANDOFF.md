...
**Written:** 2026-07-09 at project setup; **last updated 2026-08-01** — doc-only sync: `main`'s docs brought current with both open branches (`JL_Work_Tree`, `garrett/financial-eda-imputation` — neither merged; dry-runs 2026-08-01: Garrett's fast-forwards clean, JL's conflicts on 4 known files); July 22 biweekly outcome still unrecorded. (Prior updates: 2026-07-26 Product Scope Tracker; 2026-07-20 Justus's EDA 06–07.)
...
- **Milestone clock: weeks 4–6 (Concept Pitch & Wireframe)** — exit criteria: composite-score approach + dashboard wireframe presented to mentors. EDA 06/07 give the composite a head start; the wireframe has only EDA 03's choropleth as a seed.
...
- **⚠ July 22 biweekly: no outcome recorded in the repo** — no WORKLOG entry, no mentor answers, README Step 6 unticked. Mentor answers gate the composite phase (tier philosophy, weights, product shortlist, DAS vintage). Next biweekly ≈ Aug 5: reconstruct the July 22 outcome first, then bring a one-slide status (Phase 1 recap, EDA 06–09 headlines, no-catalog finding, the four composite questions).
...
1. **Coordinate the two branch merges with Justus and Garrett** (their branches; merging is a joint call with the lead — both branches read as final per their WORKLOG entries). Dry-runs (2026-08-01): `garrett/financial-eda-imputation` **fast-forwards cleanly** onto `main`; `JL_Work_Tree` conflicts on exactly README.md (pre-restored 2026-08-01 — should now resolve trivially), WORKLOG.md (`main` holds the superset — keep it), and notebooks 06+07 (add/add — take his newer versions). **After each merge:** run that branch's tests (`analysis/test_*.py` / `tests/`), re-execute its notebooks top-to-bottom locally, confirm charts regenerate — only then may its numbers enter `docs/phase1-findings-report.md`.
2. **Reconstruct and log the July 22 biweekly outcome** (ask the team; if the meeting slipped, Aug 5 becomes the concept-pitch conversation). Log mentor answers as decisions in this file and the README; tick README Step 6. Prep the ~Aug 5 one-slide status: Phase 1 recap, EDA 06–09 headlines, the no-machine-readable-catalog finding, and the four composite questions (restored in the README 2026-08-01).
3. **Composite-score phase proper + dashboard wireframe (weeks 4–6 exit criteria).** Component list with empirical justification: `docs/phase1-findings-report.md` §6 (§8 pending on `JL_Work_Tree`); EDA 06/07 are the prototype; the wireframe has only EDA 03's choropleth as a seed. Mentor-gated on #2.
4. **Garrett's CMS-scrape plan (unstarted, from 2026-07-26):** capture the `getfacets` request shape from browser DevTools; build the CMS scraper as source 5 in `FILE_CATALOG_SOURCES` (or replace the dead CKAN chain); reconcile the three counts (710 vs ~6,158 vs ~4,400 — almost certainly different units); write the "where Census data lives" explainer; spot-check the 11 `URL unverified` registry entries from a Windows browser.
...
7. Complete `docs/data-dictionary.md` (DHC, Demographic Profile, PPMF entries) once mentors confirm the product shortlist.
